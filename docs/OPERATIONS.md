...
玩家也可以在“账户与数据”下载 JSON 导出。该导出包含档案、档案操作快照、货币流水、背包流水、战斗记录和审计事件，不包含密码哈希、会话令牌或明文密码。每次成功的档案或学生操作（例如调整站位、替换学生、训练能力、招募、劝退和扩充精力）都会在同一事务中追加一份操作后的完整档案快照；失败操作不会产生快照。它适用于账户数据核对，不能替代 PostgreSQL 备份。
