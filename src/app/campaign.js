...
export function renderCampaign({ profile, selectedLevelId, message }) {
  ...
  return `<section class="app-view campaign-view" aria-labelledby="campaign-title">
    <div class="view-heading"><div><p class="eyebrow">主线关卡</p><h1 id="campaign-title">第 ${chapterNumber(selectedLevel)} 章</h1></div><p class="app-message" role="status" aria-live="polite">${esc(message)}</p></div>
    <div class="level-list" aria-label="主线关卡">${LEVELS.map((level) => {
      ...
      return `<button type="button" class="level-card${selected ? " is-selected" : ""}" data-select-level="${esc(level.id)}"${unlocked ? "" : " disabled"}><span>第 ${level.order} 关</span><strong>${esc(level.name)}</strong><small>${unlocked ? (level.recommendedTotalPower ? `推荐总战力 ${level.recommendedTotalPower}` : `推荐能力 ${level.recommendedAbility}`) : "尚未解锁"}</small></button>`;
    }).join("")}</div>
