...
export function renderArena({ profile, defense, opponents = [], match, replay, message = "" } = {}) {
  ...
  return `<section class="app-view arena-view" aria-labelledby="arena-title">
    ...
    ${match ? `<section class="panel arena-match"><div class="panel-header"><h2>比赛 ${esc(match.id)}</h2><span class="panel-meta">种子 ${esc(match.seed)}</span></div>${replay ? `<div class="settled-result ${replay.result.winner === "attacker" ? "is-win" : "is-lose"}"><h2>${replay.result.winner === "attacker" ? "挑战胜利" : replay.result.winner === "defender" ? "挑战失败" : "平局"}</h2><p>积分 ${esc(replay.rating.attackerBefore)} → ${esc(replay.rating.attackerAfter)}</p></div><details open><summary>只读战斗回放</summary><ol class="event-replay">${(replay.events?.attacker ?? []).map((event) => `<li><span>${esc(event.round ?? "准备")}</span>${esc(event.type)}</li>`).join("")}</ol></details>` : `<p class="arena-copy">双方快照已经锁定，可以开始服务器回放。</p><button class="primary-button" type="button" data-action="settle-arena">开始回放并结算</button>`}</section>` : ""}
  </section>`;
}
