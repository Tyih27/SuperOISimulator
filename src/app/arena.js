...
${match ? `<section class="panel arena-match"><div class="panel-header"><h2>对战详情</h2></div>${replay ? `<div class="settled-result ${replay.result.winner === "attacker" ? "is-win" : replay.result.winner === "draw" ? "is-draw" : "is-lose"}"><h2>${replay.result.winner === "attacker" ? "挑战胜利" : replay.result.winner === "defender" ? "挑战失败" : "平局"}</h2><p>积分 ${esc(replay.rating.attackerBefore)} → ${esc(replay.rating.attackerAfter)}</p>${replay.reward?.trainingCoins ? `<p>获得 ${esc(replay.reward.trainingCoins)} 训练币。</p>` : ""}</div><details open><summary>只读战斗回放</summary><ol class="event-replay">${(replay.events?.attacker ?? []).map((event) => `<li><span>${esc(event.round ?? "准备")}</span>${esc(EVENT_LABELS[event.type] ?? event.type)}</li>`).join("")}</ol></details>` : `${liveHtml}<p class="arena-copy">双方快照已经锁定，回放结束后将自动结算。</p>`}</section>` : ""}
  </section>`;
}
