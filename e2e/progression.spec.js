...
async function mockApi(page, options = {}) {
  ...
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (path === "/progression/training/specialist") {
      ...
      if (current.currencies.trainingCoins < 100) return json({ code: "INVALID_PROGRESSION_REQUEST", message: "Not enough training coins" }, 400);
      const prev = student.abilities[body.ability];
      const next = { ...current, version: current.version + 1, students: { ...current.students, [body.studentId]: { ...student, abilities: { ...student.abilities, [body.ability]: prev + 40 } } }, currencies: { ...current.currencies, trainingCoins: current.currencies.trainingCoins - 100 } };
