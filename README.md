...
账号、云存档、主线奖励、专项训练、每日签到、商店、招募、服务端权威单人战斗结算和玩法中心（含异步竞技场与 BOSS 战）均已实现；主线与竞技场均提供只读历史查询，主线胜利奖励首次全额、重复通关递减。生产镜像、迁移启动、健康检查、聚合监控和发布 CI 也已纳入，详见[部署文档](docs/DEPLOYMENT.md)、[安全文档](docs/SECURITY.md)和[项目完成计划](docs/superpowers/plans/2026-08-19-project-completion.md)。
...
6. 在“玩法”页的“竞技场”标签保存防守编队并挑战对手；保存后快照会随档案变动自动更新（训练、食用 KFC、招募劝退、改名、调整阵容），竞技场显示的学生数值始终与学生名单一致。进攻胜利结算 25 训练币，失败和平局不奖励训练币。每天最多主动挑战 40 场（按 `RESET_TIME_ZONE` 时区零点重置，默认 Asia/Shanghai），未结算的挑战同样占用次数，达到上限后服务端会拒绝新的挑战。结算后客户端会重新读取服务端档案，确保训练币余额同步显示。
7. 在“玩法”页的“BOSS战”标签使用当前上场编队挑战无法被击败的巨型题目，固定 30 回合；BOSS 题目难度随队伍平均战力缩放且攻击减半档（0.6 倍率），按累计造成的伤害结算训练币（每 200 点伤害 1 枚），中途全灭也保留已造成伤害。每天最多主动挑战 10 次，服务端权威结算并写入货币流水。
8. 每次成功调整站位、替换学生、训练能力、招募、劝退或扩充精力，服务端都会自动追加一份操作后的完整档案快照；失败操作不会生成快照。可在“账户与数据”导出中查看快照历史。
9. 在“账户与数据”下载导出、修改密码或删除账号（删除会立即生效并清除全部数据）。
...
│   │   ├── boss-content.js     # BOSS战关卡生成（无限血量、攻击打折）
│   │   ├── boss-engine.js      # 单侧确定性 BOSS 结算与伤害统计
│   │   ├── engine.js           # 确定性战斗引擎
│   │   ├── events.js           # 事件序列化
│   │   ├── index.js            # 战斗模块入口
│   │   └── math.js             # 战斗数值公式
