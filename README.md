...
首次启动会按 `ADMIN_USERNAME` 和 `ADMIN_PASSWORD` 创建管理员账号（默认用户名为 `admin`，默认密码为 `superoi-admin`）；如果同名账户已存在，则提升为管理员但不会覆盖其密码。请在生产环境中修改这两个配置。管理员登录后可在“账户与数据”提交反馈，并查看所有玩家反馈。
...
5. 在“学生名单”改名，在“训练与补给”进行学生强化、购买训练册或招募学生。强化按学生资质增加目标能力（普通 +15、优秀 +20、稀有 +25、天才 +30、顶尖 +40）；优先消耗对应专项训练册，没有对应训练册时可消耗 1 份学生培养材料，强化不消耗训练币。商店可购买精力药剂（150 训练币），在学生档案页使用可使该学生最大精力 +50（上限 10,000）。能力达到 2,000 上限或材料不足时不会改变档案。
...
7. 在“账户与数据”下载导出、修改密码或删除账号（删除会立即生效并清除全部数据）。
...
│   │   ├── chapter-1-3.js
│   │   ├── chapter-1-4.js
│   │   ├── chapter-2-1.js      # 额外关卡「竞赛进阶」（推荐总战力 4800-28000）
│   │   ├── chapter-2-2.js
│   │   ├── chapter-2-3.js
│   │   ├── chapter-2-4.js
